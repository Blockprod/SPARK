...
Required models (downloaded by Colab cell 3b):
...
class VideoGenerator:
    ...
    async def generate_clips(
        ...
        if not scenes:
            raise VideoGenerationError("Scenes list is empty â€” nothing to generate.")
        ...
        for scene in scenes:
            ...
            if self.cfg.use_static_fallback:
                ...
                try:
                    ...
                    LOGGER.warning(
                        "Scene %d generation failed (%s) â€” using static fallback.",
                        scene_id, exc,
                    )
                    clip_path = await asyncio.to_thread(
                        _generate_static_fallback,
        ...
        if degraded_scenes:
            LOGGER.warning(
                ...
                run_id, len(degraded_scenes), degraded_scenes,
            )
    ...
    async def _render_scene(
        ...
        def _run() -> Path:
            ...
            # Stage 1 runs at half resolution; spatial upsampler doubles it.
            stage1_h = self.cfg.height // 2
            stage1_w = self.cfg.width // 2
            ...
            # Image conditioning for inter-scene visual continuity
            if conditioning_image_path:
                try:
                    call_kwargs["images"] = [
                        (conditioning_image_path, 0, self.cfg.conditioning_strength, 33)
                    ]
                except Exception:
                    LOGGER.warning("Image conditioning setup failed â€” skipping.")

            LOGGER.info(
                "LTX-2 scene %d: %dx%d -> %dx%d, %d frames @ %.0ffps, seed=%d",
                scene_id, stage1_w, stage1_h,
                self.cfg.width, self.cfg.height,
                num_frames, self.cfg.fps, seed + scene_id,
            )
            ...
            try:
                ...
                raise VideoGenerationError(
                    f"LTX-2 generation failed for scene {scene_id}: {exc}"
                ) from exc
    ...
    def _get_pipeline(self) -> Any:
        """Lazily load the LTX-2 DistilledPipeline."""
        if self._pipeline is None:
            LOGGER.info(
                "Loading LTX-2 DistilledPipeline (fp8=%s)...", self.cfg.fp8,
            )
            ...
            try:
                ...
                quant = (
                    QuantizationPolicy.fp8_cast() if self.cfg.fp8 else None
                )
                ...
                raise VideoGenerationError(
                    f"Failed to load LTX-2 pipeline: {exc}"
                ) from exc
            LOGGER.info("LTX-2 DistilledPipeline loaded.")
...
def _compute_num_frames(duration_sec: int, fps: float) -> int:
    """Compute frame count satisfying 8k+1 constraint for LTX-2."""
